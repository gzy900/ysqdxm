  ...
  findDevice(device, devices) {
    ...
    devices.find((item) => {
      if ((item.name && item.name == device.btName) || (item.deviceId && item.deviceId == device.deviceId)) {
        if (item.advertisServiceUUIDs) { //advertisData
  ...
  CONNECT: async function (device) {
    ...
    return new Promise(async (reslove, reject) => {
            ...
            characteristics.find((it, index, array) => {
              ...
              serviceList.forEach((item, index) => {
                if (serviceId === item.serviceId) {
                  serviceList[index].characteristicId = it.uuid
                  serviceList[index].isread = true
                }
              })
              ...
              // if (it.properties.read || it.properties.notify || it.properties.indicate) {
              //   // 读取特征码
      ...
      function getBLEDeviceServices(deviceId) {
        console.log('CONNECT-getBLEDeviceServices')
        wx.getBLEDeviceServices({
          ...
          success(res) {
            ...
            services.find((it) => {
              ...
              // if (it.isPrimary && it.uuid === '0000FFB1-0000-1000-8000-00805F9B34FB') {
      ...
      function createBLEConnection() {
        console.log('CONNECT-createBLEConnection')
