    ...
    <el-table :data="dataList" style="width: 100%;margin-top:30px;" border>
      <el-table-column align="center" label="操作" width="200">
        <template slot-scope="scope">
          ...
          <el-button v-show="scope.row.audit === '1'" type="warning" size="small" @click="editState(scope,0)">冻结</el-button>
          <el-button v-show="scope.row.audit === '0'" type="danger" size="small" @click="editState(scope,1)">解冻</el-button>
        </template>
      </el-table-column>
        ...
        <el-form-item label="机构名称：" prop="organization_name">
          <el-col :span="20">
            <el-input
              type="textarea"
              :rows="3"
              v-model="fromData.organization_name"
              placeholder="请输入机构名称"
            />
          </el-col>
        </el-form-item>
...
import { getList, editData, addData } from '@/api/organization'
  ...
  methods: {
    ...
    editState({ $index, row }, state) {
      ...
      this.$confirm('确定' + msg + '吗?', '警告', {
        ...
        .then(async() => {
          await editData({ 'organizationId': row.organizationId, 'audit': state })
          this.$message({
            type: 'success',
            message: '修改成功!'
          })
    ...
    handleConfirmData(formName) {
      return new Promise((resolve, reject) => {
        if (this.dialogType === 'add') {
          delete this.fromData.organizationId
