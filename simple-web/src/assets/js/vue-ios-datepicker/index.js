...
window.datePicker = (function() {
    ...
    MobileCalendar.prototype = {
        init: function(params) {
            ...
            this.trigger = params.trigger && document.querySelector(params.trigger);
            if (this.trigger && this.trigger.getAttribute("data-lcalendar") != null) {
        ...
        bindEvent: function(type) {
            ...
            function setDateToEle(e){
                var start_date = _self.gearDate.querySelector('.start-date');
                var end_date = _self.gearDate.querySelector('.end-date');
                e.target.parentNode.childNodes.forEach(element => {
                    element.className = element.className.replace(' hover', '');
                });
                e.target.className += ' hover';
                dateMuiltCtrlInit(e.target);
                finishMobileDateMuilt(e.target);
            }
            //呼出日期(包含开始，结束时间)插件
            function popupDateMuilt(e) {
                _self.gearDate = document.createElement("div");
                _self.gearDate.className = "gearDate";
                _self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
                    '<div class="date_btn_box">' +
                    '<div class="date_btn lcalendar_cancel">取消</div>' +
                    '<div class="datemuilt-box">'+ 
                    // '<div>'+
                    '<span class="start-date hover">开始日期</span>' +
                ...
                var start_date = _self.gearDate.querySelector('.start-date');
                var end_date = _self.gearDate.querySelector('.end-date');
                dateMuiltCtrlInit(start_date);
                finishMobileDateMuilt(start_date);
                var lcalendar_cancel = _self.gearDate.querySelector(".lcalendar_cancel");
                lcalendar_cancel.addEventListener('touchstart', closeMobileCalendar);
                var lcalendar_finish = _self.gearDate.querySelector(".lcalendar_finish");
                lcalendar_finish.addEventListener('touchstart', finishMobileDateMuiltFn);
                var lcalendar_bg = _self.gearDate.querySelector(".date_bg");
                lcalendar_bg.addEventListener('click', closeMobileCalendar);

                
                start_date.addEventListener('touchstart', setDateToEle);
                end_date.addEventListener('touchstart', setDateToEle);
                ...
                //-------------------------------------------------------------
                lcalendar_cancel.addEventListener('click', closeMobileCalendar);
                lcalendar_finish.addEventListener('click', finishMobileDateMuiltFn);

                start_date.addEventListener('click', setDateToEle);
                end_date.addEventListener('click', setDateToEle);
            ...
            //初始化年月日(包含开始，结束时间)插件默认值
            function dateMuiltCtrlInit(ele) {
                var date = new Date();
                var dateArr = {
                    yy: date.getYear(),
                    mm: date.getMonth(),
                ...
                if (/^\d{4}-\d{1,2}-\d{1,2}$/.test(ele.getAttribute('val'))) {
                    rs = ele.getAttribute('val').match(/(^|-)\d{1,4}/g);
                    dateArr.yy = rs[0] - _self.minY;
                    dateArr.mm = rs[1].replace(/-/g, "") - 1;
                    dateArr.dd = rs[2].replace(/-/g, "") - 1;
                } else {
            ...
            function ymCtrlInit() {
                ...
                if (_self.trigger && /^\d{4}-\d{1,2}$/.test(_self.trigger.value)) {
                    var rs = _self.trigger.value.match(/(^|-)\d{1,4}/g);
                    dateArr.yy = rs[0] - _self.minY;
                    dateArr.mm = rs[1].replace(/-/g, "") - 1;
                } else {
            ...
            function setDateGearTooth() {
                ...
                // xh
                var hover = _self.gearDate.querySelector(".hover");
                if(hover){
                    finishMobileDateMuilt(hover);
                }
            }
            //重置时间节点个数
            ...
            function closeMobileCalendar(e) {
                ...
                _self.trigger && _self.trigger.dispatchEvent(evt);
            ...
            // muilt开始、结束日期选择
            function finishMobileDateMuilt(ele){
                var passY = _self.maxY - _self.minY + 1;
                var date_yy = parseInt(Math.round(_self.gearDate.querySelector(".date_yy").getAttribute("val")));
                var date_mm = parseInt(Math.round(_self.gearDate.querySelector(".date_mm").getAttribute("val"))) + 1;
                date_mm = date_mm > 9 ? date_mm : '0' + date_mm;
                var date_dd = parseInt(Math.round(_self.gearDate.querySelector(".date_dd").getAttribute("val"))) + 1;
                date_dd = date_dd > 9 ? date_dd : '0' + date_dd;
                ele.setAttribute("val", (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd);
            }
            // muilt日期确认
            function finishMobileDateMuiltFn(e){
                var start = _self.gearDate.querySelector(".start-date").getAttribute("val");
                var end = _self.gearDate.querySelector(".end-date").getAttribute("val");
                _self.value = { start, end };
                if(_self.onSubmit) _self.onSubmit();
                closeMobileCalendar(e);
            }
            //年月确认
            function finishMobileYM(e) {
                ...
                var date_mm = parseInt(Math.round(_self.gearDate.querySelector(".date_mm").getAttribute("val"))) + 1;
                date_mm = date_mm > 9 ? date_mm : '0' + date_mm;
                _self.trigger && (_self.trigger.value = (date_yy % passY + _self.minY) + "-" + date_mm);
                _self.value = (date_yy % passY + _self.minY) + "-" + date_mm;
                if(_self.onSubmit) _self.onSubmit();
                closeMobileCalendar(e);
            }
            //日期时间确认
