<template>
  <div class="chart-page">
    <div ref="currentChart" class="currentChart" />
...
export default {
  ...
  mounted() {
    this.getHeart([this.row], 0, 1, true)
  ...
  methods: {
    ...
    async getList() {
      this.loading = true
      const les = await getList({ uuid: this.row.uuid, flag: '1', ...this.listQuery })
    ...
    // 生成一个心电图图表
    initChart(data, row, current) {
      // 创建图表容器
      const node = this.initChartDom(current)
      // 创建图表并设置Data
      const chart = _echarts.init(node)
      this.chartsOption.title.text = row.endTime

      let subtext = 'QTC：' + row.QTC + '; ' + 'QRS：' + row.QRS + '; ' + 'HR：' + row.heart_rate_avg
      ...
      this.chartsOption.toolbox.feature.myReversal = {
        ...
        onclick: () => {
          ...
          this.chartsOption.series[0].data = list

          this.chartsOption.yAxis.min = Math.ceil(max)
          this.chartsOption.yAxis.max = Math.floor(min)

          chart.setOption(this.chartsOption)
        }
      }

      if (this.isImport !== null && this.isImport !== undefined) {
      ...
      //
      this.chartsOption.dataZoom[0].startValue = 0
      this.chartsOption.dataZoom[0].endValue = width_num * num

      this.chartsOption.dataZoom[2].startValue = 0
      this.chartsOption.dataZoom[2].endValue = width_num * num
      // 设置心电图绘制比例end
      ...
      // this.chartsOption.xAxis.data = xData
