...
import { getList } from '@/api/bp-chart'
...
export default {
  ...
  methods: {
      ...
      if (this.row) {
        this.chartsOption.series[0].markPoint = {
          data: [
            {
              name: '当前坐标',
              yAxis: this.row.high_pressure,
              xAxis: parseTime(this.row.releasetime, '{y}-{m}-{d} {h}:{i}'),
              value: '当前数据'
            }
          ]
        }
        this.chartsOption.series[1].markPoint = {
          data: [
            {
              name: '当前坐标',
              yAxis: this.row.low_pressure,
              xAxis: parseTime(this.row.releasetime, '{y}-{m}-{d} {h}:{i}'),
              value: '当前数据'
            }
          ]
        }
        this.chartsOption.series[2].markPoint = {
          data: [
            {
              name: '当前坐标',
              yAxis: this.row.heart_rate_avg,
              xAxis: parseTime(this.row.releasetime, '{y}-{m}-{d} {h}:{i}'),
              value: '当前数据'
            }
          ]
    ...
    setOption(dataList) {
      const [qtcList, qrsList, hrList, timeList] = [[], [], [], []]
      for (let i = dataList.length - 1; i >= 0; i--) {
        qtcList.push(dataList[i].high_pressure)
        qrsList.push(dataList[i].low_pressure)
