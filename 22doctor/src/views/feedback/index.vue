    ...
    <el-table :data="dataList" style="width: 100%;margin-top:30px;" border>
      ...
      <el-table-column align="center" label="操作" width="300">
        <template slot-scope="scope">
          ...
          <el-button
            v-if="scope.row.flag === '1'||scope.row.flag === '5'"
            size="small"
            @click="handleSeeReportAI(scope)"
          >移视AI报告
          </el-button>
      ...
      <el-table-column align="center" label="剩余时间" width="90">
        <template slot-scope="scope">
          <el-tag
            ...
            :type="scope.row.endTime || scope.row.releasetime | surplusType"
          >
      ...
      <el-table-column align="center" label="数据类别" width="100">
        <template slot-scope="scope">
          <el-tag :type="scope.row.flag | filterFlagType" effect="dark">
            {{ scope.row.flag | filterFlagName }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="平均心率" width="80">
        <template slot-scope="scope">
      ...
      <!--      2023-5-16 因去掉毛总报告时，删除此字段-->
      <!--      <el-table-column align="center" label="PSI" width="120">-->
      <!--        <template slot-scope="scope">-->
      <!--          <el-tag :type="scope.row.PSI | filterPSI">-->
      <!--            {{ scope.row.PSI }}-->
      <!--          </el-tag>-->
      <!--        </template>-->
      <!--      </el-table-column>-->

      <el-table-column align="center" label="SDN" width="120">
        <template slot-scope="scope">
          <el-tag :type="scope.row.heart_change | filterSDN">
            {{ scope.row.heart_change }}
          </el-tag>
        </template>
      </el-table-column>
        ...
        <el-col :span="16">
          <el-tabs v-if="dialogVisible" type="border-card" @tab-click="resizeChart">
            ...
            <template v-if="type ===7">
              <el-tab-pane label="当前数据" :lazy="true" style="overflow: hidden">
                <el-scrollbar
            ...
            <template v-if="type === 1 || type === 5">
              <el-tab-pane label="当前数据" :lazy="true">
                ...
                <!--                  <iframe name="ifd" width="100%" height="100%" style="border: 0" :src="report_url"></iframe>-->
                <!--                </div>-->
...
import ischemiaCharts from '@/components/heart-chart/ischemia-charts'
import ischemiaHistory from './charts/ischemia-history'
...
export default {
  filters: {
    ...
    filterFlagName(status) {
      ...
      return statusMap[state] || '无来源'
    },
    filterFlagType(status) {
      const state = status || 0
      const statusMap = {
        1: 'success',
        3: 'info',
  ...
  components: { Pagination, Tag, arrhythmiaHistory, heartChart, ischemiaCharts, ischemiaHistory },
  data() {
    return {
  ...
  methods: {
     ...
     */
    handleEdit(scope) {
      this.type = this.getType(scope.row.flag)
    ...
    appendImg1(data) {
      const itemWidth = 1221
      const height = 300
      const canvas = document.getElementById('imgId')
      canvas.width = itemWidth * data.length
      ...
      data.forEach((item, index) => {
        ...
        img.onload = () => {
          console.log('图片加载')
          ctx.drawImage(img, index * itemWidth, 0)
