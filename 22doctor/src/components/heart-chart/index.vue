<template>
  <div :id="returnId" class="heart-chart" :class="isError&&'is-error'" @click="clearResult">
...
// 注册必须的组件
_echarts.use(
  [TitleComponent, TooltipComponent, GridComponent, LineChart, CanvasRenderer, DataZoomComponent, ToolboxComponent, BrushComponent]
)
...
export default {
  ...
  methods: {
    ...
    setChartData(res) {
      const [icon_sava_img, icon_myReversal, icon_mySeeSmartWord, icon_mySeeWord, icon_myJia, icon_myAI, icon_myJian] = [
        ...
        'path://M478.666458 353.075869h-106.516079L135.542797 972.96732h123.979295l56.750013-154.536993H532.797908l57.623425 154.536993h123.979294L478.666458 353.075869z m-142.312576 380.665307l89.054536-243.593202 85.560889 243.593202h-174.615425zM771.152314 446.452288h116.988653V972.96732h-116.988653z',
      ...
      // 设置心电图 标题 副标题
      if (this.flag === 7) {
        option.grid.top = '34px'
        this.ch = 30
        option.title.text = '  ' + this.subTitle
      } else {
      ...
      const ch = this.chart.getHeight() - this.ch
      ...
      option.dataZoom[0].rangeMode = ['value', 'value']

      option.dataZoom[1].startValue = 0
      option.dataZoom[1].endValue = dataZoomMinX
      option.dataZoom[1].rangeMode = ['value', 'value']
    ...
    async handleSeeReport() {
      ...
      const res = await getDanDaoById({ 'report_id': this.reportId })
    ...
    async mySeeSmartWordAI() {
      // 打开移视AI报告
      console.log(this.heartId)
      if (!this.heartId) {
        this.$notify({
          title: '无智能报告！',
          type: 'warning'
      ...
      const res = await getDandaoAIReport({ 'heart_id': this.heartId })
        ...
        elements: [
          {
            ...
            zlevel: 0,
            onclick: () => {
              const cOption = this.chart.getOption()
              cOption.graphic = []
              this.chart.setOption(cOption, true)
            },
            children: [
              {
                type: 'rect',
