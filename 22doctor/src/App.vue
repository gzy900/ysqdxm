...
export default {
  metaInfo: {
    meta: [
      { name: 'viewport', content: 'upgrade-insecure-requests', 'http-equiv': 'Content-Security-Policy' }
    ]
  },
  name: 'App',
  beforeCreate() {
  ...
  created() {
    ...
    window.addEventListener('beforeunload', (e) => {
      ...
      sessionStorage.setItem('userMenus', JSON.stringify(this.$store.getters.menusList))
      if (e) {
        e.returnValue = '确认关闭？'
      } else {
        return '关闭提示'
