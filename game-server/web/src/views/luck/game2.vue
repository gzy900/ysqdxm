...
export default {
  ...
  data() {
    return {
      ...
      game_zz_src: require('../../assets/image/game-zz.png'),
  ...
  methods: {
    ...
    loop() {
      ...
      if (heroGame.x > width - width / 8) {
        heroGame.x = width - width / 8
      }
      if (heroGame.y < 0) {
        heroGame.y = 0
      }
      if (heroGame.y > height - width / 8) {
        heroGame.y = height - width / 8
      }
      ...
      for (const item in this.playerList) {
        ...
        gameBodyCtx.drawImage(player.img, player.gameData.x, player.gameData.y, width / 8, width / 8)

        if (this.startGameStatus) {
          gameBgCtx.beginPath();
          gameBgCtx.arc(player.gameData.x + width / 16, player.gameData.y + width / 16, width / 16, 0, 2 * Math.PI);
          gameBgCtx.fillStyle = player.color;
          gameBgCtx.fill();
        }
      }
      ...
      // 绘制自己英雄
      gameBodyCtx.drawImage(heroGame.img, heroGame.x, heroGame.y, width / 8, width / 8)

      // 绘制自己的路径
      if (this.startGameStatus) {
        gameBgCtx.beginPath();
        gameBgCtx.arc(heroGame.x + width / 16, heroGame.y + width / 16, width / 16, 0, 2 * Math.PI);
        gameBgCtx.fillStyle = heroGame.color;
        gameBgCtx.fill();
      }

      if (game_zz_data.status) {
        gameBgCtx.drawImage(game_zz, game_zz_data.x, game_zz_data.y, width / 8, width / 8)
      }
    },
    touchstart(e) {
    ...
    connection() {
      ...
      this.ws.onmessage = function (e) {
        ...
        // 获取中奖信息
        if (msg.type === '5' && msg.msg.luckType === '2') {
          game_zz_data.status = true
          game_zz_data.x = msg.msg.luck[0] * width
          game_zz_data.y = msg.msg.luck[1] * height
        }
      };
    },
