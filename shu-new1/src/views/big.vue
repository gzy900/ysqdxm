<template>
  <div class="container shu">
    <p class="map-mask"></p>
    <div id="mapChart" class="mapChart" style="display: none"></div>
    <div id="linesChart" class="linesChart" style="display: none"></div>
    ...
    <div class="content">
      <div class="left">
        ...
        <div class="title">
          <span class="textColor">移视科技心电一张网全网数据中心</span>
        ...
        <div class="left-top">
          ...
          <div class="title-data">
            <p class="title-data-name">设备总数</p>
            <p class="title-data-num">{{ info.data_statistics[0]?.value }}<span style="font-size: 16px">套</span></p>
            <span class="line1"></span>
          </div>

          <div class="title-data">
            <span class="title-data-name">当日活跃数量</span>
            <span class="title-data-num">{{ info.data_statistics[1]?.value }}<span style="font-size: 16px">套</span></span>
            <span class="line1"></span>
          </div>

          <div class="title-data">
            <span class="title-data-name">心电图</span>
            <span class="title-data-num">{{ info.data_statistics[3]?.value * 2160 }}<span style="font-size: 16px">米</span></span>
            <span class="line1"></span>
          </div>

          <div class="title-data">
            <span class="title-data-name">血压</span>
            <span class="title-data-num">{{ info.data_statistics[3]?.value * 35 }}<span style="font-size: 16px">人</span></span>
            <span class="line1"></span>
          </div>

          <div class="title-data">
            <span class="title-data-name">血氧</span>
            <span class="title-data-num">{{ info.data_statistics[3]?.value * 24 }}<span style="font-size: 16px">小时</span></span>
            <span class="line1"></span>
          </div>
        </div>
        <div class="left-bottom">
          <div class="left-doctor">
            <div class="doctor-body">
              <p style="margin-bottom: 17px;  color: #2be7f8;font-size: 18px;font-weight: bold;">
                <span class="rank"> </span>
                <span class="name">医生</span>
                <span class="num">昨日判图明星</span>
              </p>
              <!--              <p v-for="(item,index) in info.data_doctor">-->
              <!--                <span class="rank">{{ index + 1 }}</span>-->
              <!--                <span class="name">{{ item.title }}</span>-->
              <!--                <span class="num">-->
              <!--                  <span :style="{width: (item.value/info.data_doctor[0].value*100)+'%'}" class="num-value">{{ item.value }}</span>-->
              <!--                </span>-->
              <!--              </p>-->

              <div class="data-list" :style="{height:doctorHeight+'px'}" ref="doctorItem">

                <p v-for="(item,index) in info.data_doctor" :style="{transition: 'transform '+progObj['d'].speed+'s', transform: 'translate3d(0,-' + (progObj['d'].moveHeight) + 'px,0)' }">
                  <span class="ranking">{{ index + 1 }}</span>
                  <span class="data-item-value">
                <span class="name">{{ item.title + ' ' }}<span class='bold-num'>{{ item.value }}</span></span>
                <span class="progress">
                  <progressC rgba="#6758fa" :run="run(index,info.data_doctor.length)" :value="item.value/info.data_doctor[0].value*100" class="progressDom"></progressC>
                </span>
                </span>
                  <!--                24012-->
                </p>
                ...
                <span class="name">{{ item.title + ' ' }}<span class='bold-num'>{{ item.value }}</span></span>
                <span class="progress">
                  <progressC rgba="#6758fa" :run="run(index+info.data_doctor.length,info.data_doctor.length)" :value="item.value/info.data_doctor[0].value*100" class="progressDom"></progressC>
                </span>
                </span>
                  <!--                24012-->
                </p>
              ...
              <div style="margin-top: 20px;font-size: 16px">
                <span style="display: block">判图总数：2022年至今</span>
                <span style="display: inline-block;font-size: 24px;color: #fabf67">{{ info.data_statistics[2]?.value }}</span> 份
              </div>
            </div>
          </div>
      ...
      <div class="right">
        ...
        <div class="right-body">
          <div class="right-data-body">
            <p class="title">
              2023年服务患者人数总量 <span style="font-size: 14px;color: #fabf67">{{ info.data_statistics[3]?.value + '/次' }}</span>
            </p>
            <div class="data-list" ref="reportItem">

              <p v-for="(item,index) in info.data_serving_patients" :style="{transition: 'transform '+progObj['f'].speed+'s', transform: 'translate3d(0,-' + (progObj['f'].moveHeight) + 'px,0)' }">
                <span class="ranking">{{ index + 1 }}</span>
                <span class="data-item-value">
                <span class="name">{{ item.title + ' ' }}<span class='bold-num'>{{ item.value }}</span></span>
                <span class="progress">
                  <progressC rgba="#6758fa" :run="run(index,info.data_serving_patients.length)" :value="item.value/info.data_serving_patients[0].value*100" class="progressDom"></progressC>
                </span>
                </span>
                <!--                24012-->
              </p>
              ...
              <p v-for="(item,index) in info.data_serving_patients" :style="{transition: 'transform '+progObj['f'].speed+'s', transform: 'translate3d(0,-' + (progObj['f'].moveHeight) + 'px,0)' }">
                ...
                <span class="name">{{ item.title + ' ' }}<span class='bold-num'>{{ item.value }}</span></span>
                <span class="progress">
                  <progressC rgba="#6758fa" :run="run(index+info.data_serving_patients.length,info.data_serving_patients.length)" :value="item.value/info.data_serving_patients[0].value*100" class="progressDom"></progressC>
                </span>
                </span>
                <!--                24012-->
              </p>
          ...
          <div class="right-data-body">
            <p class="title">
              活跃设备数量排名 <span style="font-size: 14px;color: #fabf67">昨日</span>
            </p>
            <div class="data-list shebei">
              <!--              <p v-for="(item,index) in active" :style="{transition: 'transform '+sustain+'s', transform: 'translate3d(0,-' + (reportHeight) + 'px,0)' }">-->
              <!--                <span class="ranking">{{ index % (active.length / 2) + 1 }}</span>-->
              <!--                <span class="data-item-value">-->

              <!--                <span class="name">{{ item.name + ' ' + item.value }}</span>-->
              <!--                <span class="progress">-->
              <!--                  <progressC :run="run(index)" :value="item.value/active[0].value*100" class="progressDom"></progressC>-->
              <!--                </span>-->
              <!--                </span>-->
              <!--              </p>-->
              <p v-for="(item,index) in info.data_active_devices" :style="{transition: 'transform '+progObj['h'].speed+'s', transform: 'translate3d(0,-' + (progObj['h'].moveHeight) + 'px,0)' }">
                <span class="ranking">{{ index + 1 }}</span>
                <span class="data-item-value">

                <span class="name">{{ item.title + ' ' }}<span class='bold-num'>{{ item.value }}</span></span>
                <span class="progress">
                  <progressC :run="run(index,info.data_active_devices.length)" :value="item.value/info.data_active_devices[0].value*100" class="progressDom"></progressC>
                </span>
                </span>
              </p>
              <p v-for="(item,index) in info.data_active_devices" :style="{transition: 'transform '+progObj['h'].speed+'s', transform: 'translate3d(0,-' + (progObj['h'].moveHeight) + 'px,0)' }">
                ...
                <span class="name">{{ item.title + ' ' }}<span class='bold-num'>{{ item.value }}</span></span>
                <span class="progress">
                  <progressC :run="run(index+info.data_active_devices.length,info.data_active_devices.length)" :value="item.value/info.data_active_devices[0].value*100" class="progressDom"></progressC>
                </span>
                </span>
              </p>
          ...
          <div class="right-data-body">
            ...
            <div class="data-list hospital" ref="hospital">
              <div style="width: 100%;height: 100%" :style="{transition: 'transform '+progObj['y'].speed+'s', transform: 'translate3d(0,-' + (progObj['y'].moveHeight) + 'px,0)' }">
                <p v-for="(item,index) in info.data_signed_hospital" :class="loopIndex%info.data_signed_hospital.length+1<=index&&loopIndex%info.data_signed_hospital.length+5+1>index?'active':''">
                  <span v-for="i in 10" class="name" :data="item.title">{{ item.title }}</span>
                  <!--                <span class="ranking">30件</span>-->
                </p>
                <p v-for="(item,index) in info.data_signed_hospital" :class="loopIndex%info.data_signed_hospital.length+1<=index+info.data_signed_hospital.length&&loopIndex%info.data_signed_hospital.length+5+1>index+info.data_signed_hospital.length?'active':''">
                  <span v-for="i in 10" class="name" :data="item.title">{{ item.title }}</span>
                  <!--                <span class="ranking">30件</span>-->
                </p>
              </div>
    ...
    <map3d v-if="info.data_all_city.length>0" class="map3d" :data="info.data_all_city"></map3d>
    <div id="container" style="z-index:9;width: 100%;height: 100%;position: absolute;top: 0;left: 0"></div>
...
// import clouds from './bg.vue'
import clouds from '../components/bg3d.vue'
import progressC from '../components/progress.vue'
import { option, linesOption } from './big/option.js'
import { requestFiles } from './big/requestGeoJson.js'
import url from '/public/bg4.png'
import map3d from '../components/map.vue'
import { getData } from "../api/big.js";

let timeData
const reportItem = ref(null)
const hospital = ref(null)
let hospitalRect = ref(0)
let reportIndex = ref(0)
let reportHeight = ref(0)
let doctorHeight = ref(0)
let reportRect
...
let styleComputed = computed({
  get() {
    return (value) => {
      let style
      style = { transform: 'translate3d(0,-' + (reportHeight.value) + 'px,0)' }
      // if (reportIndex.value === report.value.length) {
      //   reportIndex.value = 0
      // style = { transition: 'transform 0s', transform: 'translate3d(0,-' + (reportHeight.value) + 'px,0)' }
...
function loop() {
  reportRect = reportItem.value.getBoundingClientRect()
  hospitalRect = ref(reportItem.value.getBoundingClientRect().height)
  console.log(hospitalRect)
  console.log(reportRect)
  console.log(reportItem.value.clientHeight)
  doctorHeight = reportRect.height
  progObj.value.d = {
    length: info.data_doctor.length,
    boxHeight: reportRect.height,
    moveHeight: reportRect.height / 5,
    speed: .3
  }
  progObj.value.f = {
    length: info.data_serving_patients.length,
    boxHeight: reportRect.height,
    moveHeight: reportRect.height / 5,
    speed: .3
  }
  progObj.value.h = {
    length: info.data_active_devices.length,
    boxHeight: reportRect.height,
    moveHeight: reportRect.height / 5,
    speed: .3
  }
  progObj.value.y = {
    length: info.data_signed_hospital.length,
    boxHeight: hospitalRect,
    moveHeight: hospitalRect / 5,
    speed: .3
  }
  timeData = setInterval(() => {
    loopIndex.value++
    for (let i in progObj.value) {
      const item = progObj.value[i]
      item.speed = .3
      item.moveHeight = item.boxHeight / 5 * (loopIndex.value % item.length + 1)
      if (loopIndex.value % (item.length) === item.length - 1) {
...
let run = computed({
  get() {
    return (value, len) => {
      let flag
      let el = 1
      if (value < loopIndex.value % len + 5 + el) {
        flag = 1
      } else {
        flag = 0
      }
      if (value === 5 && loopIndex.value % len > value) {
        flag = 0
      }
      return flag
    }
...
function loopReport() {
  ...
  timeData = setInterval(() => {
    reportIndex.value++
    reportHeight.value = reportIndex.value * (reportRect.height / 5)
    sustain = .3
    if (reportIndex.value === report.value.length / 2) {
      let time = setTimeout(() => {
        sustain = 0
        reportIndex.value = 0
        reportHeight.value = reportIndex.value * (reportRect.height / 5)
...
const default_info = {
  data_doctor: [],
  data_serving_patients: [],
  data_active_devices: [],
  data_signed_hospital: [],
  data_statistics: [],
  data_all_city: []
}

let info = reactive(default_info)
...
function initData(Content) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (item.title !== '名字') {
      let dataNameNew = 0
      for (let d_item in info) {
        if (d_item === item.title) {
          dataNameNew = 1
          dataName = item.title
        }
      }
      if (!dataNameNew) {
        info[dataName].push(item)
      }
    }
  }
...
function initEchart() {
  echarts.registerMap('中国省', {
    type: "FeatureCollection",
    features: [{ id: "710000", type: "Feature", geometry: { type: "MultiPolygon", coordinates: [["@@°Ü¯Û"], ["@@ƛĴÕƊÉɼģºðʀ\\ƎsÆNŌÔĚänÜƤɊĂǀĆĴĤǊŨxĚĮǂƺòƌâÔ®ĮXŦţƸZûÐƕƑGđ¨ĭMó·ęcëƝɉlÝƯֹÅŃ^Ó·śŃǋƏďíåɛGɉ¿@ăƑ¥ĘWǬÏĶŁâ"], ["@@\\p|WoYG¿¥Ij@¢"], ["@@¡@V^RqBbAnTXeRz¤L«³I"], ["@@ÆEEkWqë @"], ["@@fced"]], encodeOffsets: [[[122886, 24033]], [[123335, 22980]], [[122375, 24193]], [[122518, 24117]], [[124427, 22618]], [[124862, 26043]]] }, properties: { cp: [121.509062, 25.044332], name: "台湾", childNum: 6 } }, {
    ...
    }, { id: "140000", type: "Feature", geometry: { type: "Polygon", coordinates: ["@@ÞĩÒSra}ÁyWix±Üe´lèßÓǏokćiµVZģ¡coTSË¹ĪmnÕńehZg{gtwªpXaĚThȑp{¶Eh®RćƑP¿£Pmc¸mQÝWďȥoÅîɡųAďä³aÏJ½¥PG­ąSM­EÅruµéYÓŌ_dĒCo­Èµ]¯_²ÕjāK~©ÅØ^ÔkïçămÏk]­±cÝ¯ÑÃmQÍ~_apm~ç¡qu{JÅŧ·Ls}EyÁÆcI{¤IiCfUcƌÃp§]ě«vD@¡SÀµMÅwuYY¡DbÑc¡h×]nkoQdaMç~eDÛtT©±@¥ù@É¡ZcW|WqOJmĩl«ħşvOÓ«IqăV¥D[mI~Ó¢cehiÍ]Ɠ~ĥqX·eƷn±}v[ěďŕ]_œ`¹§ÕōIo©b­s^}Ét±ū«³p£ÿ·Wµ|¡¥ăFÏs×¥ŅxÊdÒ{ºvĴÎêÌɊ²¶ü¨|ÞƸµȲLLúÉƎ¤ϊęĔV`_bªS^|dzY|dz¥pZbÆ£¶ÒK}tĦÔņƠPYznÍvX¶Ěn ĠÔzý¦ª÷ÑĸÙUȌ¸dòÜJð´ìúNM¬XZ´¤ŊǸ_tldI{¦ƀðĠȤ¥NehXnYGR° ƬDj¬¸|CĞKqºfƐiĺ©ª~ĆOQª ¤@ìǦɌ²æBÊTŸʂōĖĴŞȀÆÿȄlŤĒötÎ½î¼ĨXh|ªM¤Ðz"], encodeOffsets: [[116874, 41716]] }, properties: { cp: [112.549248, 37.857014], name: "山西", childNum: 1 } }, {
    ...
    }, { id: "460000", type: "Feature", geometry: { type: "Polygon", coordinates: ["@@¦Ŝil¢XƦƞòïè§ŞCêɕrŧůÇąĻõ·ĉ³œ̅kÇm@ċȧŧĥĽʉ­ƅſȓÒË¦ŝE}ºƑ[ÍĜȋ gÎfǐÏĤ¨êƺ\\Ɔ¸ĠĎvʄȀÐ¾jNðĀÒRZǆzÐŘÎ°H¨Ƣb²_Ġ "], encodeOffsets: [[112750, 20508]] }, properties: { cp: [110.33119, 20.031971], name: "海南", childNum: 1 } }, {
    ...
    }, { id: "110000", type: "Feature", geometry: { type: "Polygon", coordinates: ["@@ĽOÁûtŷmiÍt_H»Ĩ±d`¹­{bwYr³S]§§o¹qGtm_SŧoaFLgQN_dV@Zom_ć\\ßcÂ±x¯oœRcfe£o§ËgToÛJíĔóu|wP¤XnO¢ÉŦ¯rNÄā¤zâŖÈRpŢZÚ{GrFt¦Òx§ø¹RóäV¤XdżâºWbwŚ¨Ud®bêņ¾jnŎGŃŶnzÚSeîĜZczî¾i]ÍQaúÍÔiþĩȨWĢü|Ėu[qb[swP@ÅğP¿{\\¥A¨ÏÑ¨j¯X\\¯MKpA³[Hīu}}"], encodeOffsets: [[120023, 41045]] }, properties: { cp: [116.405285, 39.904989], name: "北京", childNum: 1 } }, { id: "120000", type: "Feature", geometry: { type: "Polygon", coordinates: ["@@ŬgX§Ü«E¶FÌ¬O_ïlÁgz±AXeµÄĵ{¶]gitgIj·¥îakS¨ÐƎk}ĕ{gBqGf{¿aU^fIư³õ{YıëNĿk©ïËZŏR§òoY×Ógcĥs¡bġ«@dekąI[nlPqCnp{ō³°`{PNdƗqSÄĻNNâyj]äÒD ĬH°Æ]~¡HO¾X}ÐxgpgWrDGpù^LrzWxZ^¨´T\\|~@IzbĤjeĊªz£®ĔvěLmV¾Ô_ÈNW~zbĬvG²ZmDM~~"], encodeOffsets: [[120237, 41215]] }, properties: { cp: [117.190182, 39.125596], name: "天津", childNum: 1 } }, {
      id: "310000",
      type: "Feature",
      geometry: { type: "MultiPolygon", coordinates: [["@@ɧư¬EpƸÁxc"], ["@@©ª"], ["@@MA"], ["@@QpİE§ÉC¾"], ["@@bŝÕÕEȣÚƥêImɇǦèÜĠÚÃƌÃ͎ó"], ["@@ǜûȬɋŭ×^sYɍDŋŽąñCG²«ªč@h_p¯A{oloY¬j@Ĳ`gQÚhr|ǀ^MĲvtbe´R¯Ô¬¨Yô¤r]ìƬį"]], encodeOffsets: [[[124702, 32062]], [[124547, 32200]], [[124808, 31991]], [[124726, 32110]], [[124903, 32376]], [[124438, 32149]]] },
    ...
    }, { id: "810000", type: "Feature", geometry: { type: "MultiPolygon", coordinates: [["@@AlBk"], ["@@mn"], ["@@EpFo"], ["@@ea¢pl¸Eõ¹hj[]ÔCÎ@lj¡uBX´AI¹[yDU]W`çwZkmcMpÅv}IoJlcafŃK°ä¬XJmÐ đhI®æÔtSHnEÒrÈc"], ["@@rMUwAS®e"]], encodeOffsets: [[[117111, 23002]], [[117072, 22876]], [[117045, 22887]], [[116975, 23082]], [[116882, 22747]]] }, properties: { cp: [114.173355, 22.320048], name: "香港", childNum: 5 } }, { id: "820000", type: "Feature", geometry: { type: "Polygon", coordinates: ["@@kÊd°å§s"], encodeOffsets: [[116279, 22639]] }, properties: { cp: [113.54909, 22.198951], name: "澳门", childNum: 1 } }], UTF8Encoding: !0
  })
  ...
  // const router = [
  //   [
  //     [100.456984, 38.930493, 2, 5],
  //     [121.475896, 31.299892, 3, 5],  // 起点
  //   ]
  // ]

  linesOption.series[0].data = router

  linesOption.geo3D.colorMaterial.detailTexture = img.value
...
.doctor-body {
  ...
  position: relative;
  color: #ffffff;
  box-shadow: 0 0 8px 2px rgba(43, 230, 248, 1);

  padding: 15px;
  padding-bottom: 24px;
}

.doctor-body p:first-child {
}

.doctor-body p:first-child .num {
  height: auto;
...
.doctor-body p:first-child .rank {
  text-indent: 0;
...
.textColor {
  /*font-size: 36px;*/
  font-weight: bold;
  background-image: linear-gradient(to right, #0dc7f5, #6758fa, #fff, #fdc167, #fff, #6758fa, #0dc7f5, #6758fa, #fff, #fdc167, #fff, #6758fa, #0dc7f5);
...
.title-data-name {
  color: #2be7f8;
  font-size: 36px;
  text-shadow: 0 0 5px #2be7f8;

  /*text-shadow: 5px 5px 2px #666, 6px 6px 2px #eee;*/
}
...
.line1 {
  width: 100%;
  height: 4px;
  /*background-color: linear-gradient(to right, transparent, #0dc3f1, transparent);*/
  background: linear-gradient(to right, #0dc3f1, transparent);
  display: inline-block;
}
...
.line {
  ...
  height: 3px;
  /*background-color: linear-gradient(to right, transparent, #0dc3f1, transparent);*/
  background: linear-gradient(to right, transparent, #0dc3f1, transparent);
  display: inline-block;
}
...
.hospital div p {
  ...
  animation: wobble 1s ease-in-out 1 forwards;
  transform-origin: center center;
  transform-style: preserve-3d;
  perspective: 500px;
  height: 20%;
  transform: rotate3d(-1, 1, 0, 40deg) scale3d(1.2, 1.2, 1.2);
  transition: transform 1s;
}

.hospital div .active {
  transform: rotate3d(1, 0, 0, 20deg) scale3d(1, 1, 1);
...
.hospital .name {
  ...
  text-shadow: 0 0 5px rgba(43, 231, 248, 1),
  0 0 15px rgba(43, 231, 248, 1);
...
.shu .data-list .data-item-value {
  height: 100%;
  flex-direction: column;
}

.shu .data-list .data-item-value .name {
  width: 100%;
  font-size: 12px;
}
...
.hospital p {
  ...
  justify-content: space-between;
  /*transform: scale(1.5);*/
  /*animation: lysis2 2s infinite forwards;*/
...
.shu .data-list .progress {
  padding: 0;
  width: 100%;
}

.data-list .progress .progress-value {
  display: inline-block;
  border-radius: 20px;
  height: 14px;
}
...
.data-list .progress .progressDom {;
  position: relative;
  top: 50%;
...
.right-data-body {
  ...
  border-image: linear-gradient(to right, transparent, #0dc3f1, transparent);
  border-bottom: 4px solid;
}

.right-data-body .line {
  margin-top: 15px;
}

.right-data-body .title {
  color: #2be6f8;
  padding-bottom: 10px;
  font-size: 18px;
  flex: none;
...
.map-mask {
  ...
  /*box-shadow: 0 0 600px 220px rgba(255, 255, 255, 1);*/
  /*box-shadow: 0 0 500px rgba(33, 53, 71, 1);*/
}
